...
class EmotionObject{
    ...
    getOnomatopoeiaWords(){
        ...
        for (let i = 0; i < this.onomatopoeia.length; i++) { 
            result.push(new Word (this.onomatopoeia[i], dictionary[this.onomatopoeia[i]], pictureDictionary[this.onomatopoeia[i]]));
        }
        return result;
    }
    ...
    getHtmlContainerString(){
        ...
        for(let i=0;i<this.getOnomatopoeiaWords().length;i++){
            let key = this.getOnomatopoeiaWords()[i]
            containerSectionString +=
             `
                <div class="d-flex justify-content-between flex-wrap">
                    <div class="d-flex col-md-5 col-12 bg-white px-0 my-2">
                        ...
                        <div class="col-4 px-4 d-flex justify-content-center align-items-center">
                            <img class="col-12 imgFit p-1" src="${key.pictureUrl}">
                        </div>
                    </div>
                </div>
...
const emotions = [
    new EmotionObject("angry","(( ｀ー´)ノ", "feeling or showing strong annoyance, displeasure, or hostility; full of anger.", "red", ["bark","grunt", "roar","whack","smack","hiss"]),
    new EmotionObject("happy","(^^)/", "feeling or showing pleasure or contentment.", "pink", ["bling","chatter","chant","giggle"]),
    new EmotionObject("bad", "(；ﾟДﾟ)","not such as to be hoped for or desired; unpleasant or unwelcome.", "lightblue", ["ahem","clatter","clunk"]),
    new EmotionObject("sad","(◞‸◟)","feeling or showing sorrow; unhappy.", "grey", ["bawl","whine","waah"]),
    new EmotionObject("surprised","(@_@)", "to feel mild astonishment or shock.", "purple", ["boom","honk","zing"]),
    new EmotionObject("fearful","(/ω＼)", "feeling afraid; showing fear or anxiety.", "green", ["buzz","caw","crawl"]),
    new EmotionObject("disgusted","(ToT)/~~~", "feeling or showing strong annoyance, displeasure, or hostility; full of anger.", "orange", ["flick","gargle","oink"])
];
...
for(let i=0;i<emotions.length;i++){
    ...
    `
    <div class="container">
        <a class="text-white" style="text-decoration:none" href="#${current.emotion}">
        <div class="text-white text-center py-3 p-4 m-4" style="background:${current.color}">
...
for(let i=0;i<emotions.length;i++){
    content += emotions[i].getHtmlContainerString();
}
